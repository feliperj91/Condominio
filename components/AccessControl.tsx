...
export const AccessControl: React.FC = () => {
    ...
    const loadPermissions = async () => {
        try {
            setIsLoading(true);
            // Check if getPermissions exists (it might not in mock service)
    ...
    const roles = Object.keys(groupedPermissions) as Role[];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {roles.map(role => (
                    <div key={role} className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                        <h4 className="font-bold text-slate-800 mb-4 border-b pb-2 flex justify-between items-center">
                            ...
                            <span className="text-xs font-normal text-slate-500 bg-slate-100 px-2 py-1 rounded">
                                {groupedPermissions[role].length} recursos
                            </span>
                        </h4>
